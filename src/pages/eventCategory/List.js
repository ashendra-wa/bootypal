  ...
  async function getList() {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/api/eventcategory/listAll?enabled=true&sortBy=enabled`,
        {
          headers: {
  ...
  async function handleDelete(id) {
    try {
      const response = await axios.delete(
        `${baseUrl}/api/eventcategory/delete/${id}`,
        {
          headers: {
        ...
        <Row gutter={[24, 0]}>
          <Col xs="24" xl={24}>
            <Card
              ...
              extra={
                <>
                  <Link className="custom-btn" to="/event-category/add">
                    Add
                  </Link>
                </>
              }
                  ...
                  dataSource={userList.map((user, index) => ({
                    ...
                    action: (
                      <div className="button-container">
                        <Link
                          to={`/event-category/update/${user._id}`}
