...
function BlogList() {
    ...
    async function getUserList() {
        try {
            const response = await axios.get(`${baseUrl}/api/blog-category/list`, {
                params: {
                    page: currentPage,
                    limit: pageSize,
                },
    ...
    async function handleDelete(id) {
        try {
            const response = await axios.delete(`${baseUrl}/api/blog-category/delete/${id}`, {
                headers: {
                ...
                <Row gutter={[24, 0]}>
                    <Col xs="24" xl={24}>
                        <Card
                            ...
                            extra={
                                <>
                                    <Link className="custom-btn" to="/blog-category/add">Add</Link>
                                </>
                            }
                        >
                                    ...
                                    dataSource={userList.map((user, index) => ({
                                        ...
                                        action: (
                                            <div className="button-container">
                                                <Link to={`/blog-category/update/${user._id}`} className="update-btn">
                                                    <EditOutlined />
                                                </Link>
