...
function HobbiesList() {
    ...
    async function getList() {
        try {
            ...
            const response = await axios.get(`${baseUrl}/api/foodpreference/listAll`, {
                headers: {
    ...
    async function handleDelete(id) {
        try {
            const response = await axios.delete(`${baseUrl}/api/foodpreference/delete/${id}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('accessToken')}`, // Include access token in headers
                },
            });
                ...
                <Row gutter={[24, 0]}>
                    <Col xs="24" xl={24}>
                        <Card
                            ...
                            extra={
                                <>
                                    <Link className="custom-btn" to="/food-preference/add">Add</Link>
                                </>
                            }
                        >
                                    ...
                                    dataSource={userList.map((user, index) => ({
                                        ...
                                        action: (
                                            <div className="button-container">
                                                <Link to={`/food-preference/update/${user._id}`} className="update-btn">
                                                    <EditOutlined />
                                                </Link>
