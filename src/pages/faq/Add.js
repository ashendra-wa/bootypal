...
function FaqAdd() {
    ...
    const onFinish = async (values) => {
        try {
            if (isUpdateMode) {
                const response = await axios.patch(`${baseUrl}/api/faq/update/${faqId}`, values, {
                    headers: {
    ...
    const onFinishFailed = ({ errorFields }) => {
        ...
        const hasAnswerError = errorFields.some(field => field.name[0] === 'answer');
        if (hasAnswerError) {
        ...
        <div className="tabled">
            <Row gutter={[24, 0]}>
                <Col xs="24" xl={24}>
                    ...
                    >
                        <Form style={{ "padding": "20px" }} form={form} onFinish={onFinish} onFinishFailed={onFinishFailed} layout="vertical">
                            <Row gutter={[16, 16]}>
                                ...
                                <Col xs={24} sm={12} lg={12}>
                                    ...
                                    >
                                        <ReactQuill style={{
                                            border: answerError ? '1px solid red' : '1px solid #d9d9d9',
                                            borderRadius: 6,
                                            // padding: 2
                                        }} />
                                    </Item>
